...
from football_match_notification_service.models import Event, Match, EventType, MatchStatus
...
class EventDetector:
    ...
    def detect_events(self, match: Match) -> List[Event]:
        ...
        if previous_match:
            logger.debug(f"Match status: previous={previous_match.status}, current={match.status}")
        ...
        logger.info(f"Found {len(events)} events, {len(unique_events)} unique for match {match.id}")
        if events:
    ...
    def _is_match_start(self, current: Match, previous: Optional[Match]) -> bool:
        ...
        return (previous.status != "IN_PLAY" and 
                current.status == "IN_PLAY")
    ...
    def _is_half_time(self, current: Match, previous: Optional[Match]) -> bool:
        ...
        # Debug logging
        logger.debug(f"Half-time check: previous={previous.status}, current={current.status}")
    ...
    def _is_match_end(self, current: Match, previous: Optional[Match]) -> bool:
        ...
        # Debug logging
        logger.debug(f"Match end check: previous={prev_status}, current={curr_status}, result={result}")
    ...
    def _create_goal_event(self, match: Match, is_home_team: bool, goal_number: int) -> Event:
        """
        Create a goal event.
    ...
    def _filter_duplicate_events(self, events: List[Event]) -> List[Event]:
        ...
        if len(unique_events) != len(events):
            logger.debug(f"Filtered out {len(events) - len(unique_events)} duplicate events")
