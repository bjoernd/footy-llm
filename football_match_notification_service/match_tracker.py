...
class MatchTracker:
        ...
        for team in self.teams_to_track:
            try:
                ...
                for match in matches:
                    ...
                    if (match_id not in self.upcoming_matches and
                        match_id not in self.active_matches and
                        match_id not in self.recent_matches):
                        
                        self.upcoming_matches[match_id] = match
                        new_matches.append(match)
                        logger.info(f"Discovered new match: {match.home_team.name} vs {match.away_team.name}")
                        
            except Exception as e:
                logger.error(f"Error discovering matches for team {team.name}: {str(e)}")
    ...
    def update_match_status(self, match_id: str) -> Tuple[Optional[Match], bool]:
        ...
        try:
            ...
            elif updated_match.status in [MatchStatus.IN_PLAY, MatchStatus.PAUSED, MatchStatus.SUSPENDED]:
            ...
            if status_changed:
                logger.info(f"Match status changed: {updated_match.home_team.name} vs {updated_match.away_team.name} - {updated_match.status}")
...
def get_match_tracker(
    ...
    if _match_tracker_instance is None:
        from football_match_notification_service.config_manager import get_config_manager
        
        if api_client is None:
            from football_match_notification_service.api_client import get_football_data_client
