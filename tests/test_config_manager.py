...
class TestConfigManager:
    ...
    def test_load_config_valid_json(self):
        ...
        test_config = {
            "teams": [{"name": "Test Team", "league": "Test League", "team_id": "123"}],
            "api_settings": {"api_key": "test_key", "base_url": "https://test.api"},
            "notification_preferences": {"channels": ["email"]},
            "polling_settings": {"frequency_normal": 300},
        }
    ...
    def test_validate_config_missing_fields(self):
        ...
        try:
            ...
            assert (
                "Section 'notification_preferences' missing required field: channels"
                in errors
            )
            assert (
                "Section 'polling_settings' missing required field: frequency_normal"
                in errors
            )
        finally:
            # Clean up
    ...
    def test_get_config_value(self):
        ...
        try:
            ...
            assert (
                config_manager.get("api_settings.non_existent", "default") == "default"
            )
        finally:
            # Clean up
            os.unlink(temp_path)
    ...
    def test_get_with_default_config(self):
        ...
        try:
            ...
            assert (
                config_manager.get_with_default(
                    "polling_settings.frequency_during_match"
                )
                == 60
            )
            assert config_manager.get_with_default(
                "notification_preferences.priority_order"
            ) == ["email", "sms", "signal"]

            # Test getting user-specified values
            assert config_manager.get_with_default("api_settings.api_key") == "test_key"
            assert (
                config_manager.get_with_default("polling_settings.frequency_normal")
                == 300
            )
        finally:
            # Clean up
    ...
    def test_reload_config(self):
        ...
        initial_config = {
            "teams": [
                {"name": "Initial Team", "league": "Initial League", "team_id": "123"}
            ],
            "api_settings": {
                "api_key": "initial_key",
        ...
        updated_config = {
            "teams": [
                {"name": "Updated Team", "league": "Updated League", "team_id": "456"}
            ],
            "api_settings": {
                "api_key": "updated_key",
                "base_url": "https://updated.api",
            },
            "notification_preferences": {"channels": ["sms"]},
            "polling_settings": {"frequency_normal": 600},
        }
